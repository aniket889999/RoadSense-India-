...
2. Runs local, hardware-accelerated computer vision suggestions (using a frozen, provenance-verified YOLOv8n detector).
3. Consolidates repeated, frame-by-frame raw bounding box detections into stable, reviewable **Road Events**.
4. Provides human inspectors with an evidence-backed review workbench to confirm, reject, or mark suggestions for revisit.
5. Exports auditable, path-free field inspection dossiers and CSV/JSON packages for enterprise GIS and maintenance tracking.
...
- **Workflow**:
  ...
  2. Watches sampled playback with highlighted suggestion markers.
...
| **Accident / Hazard Risk** | Highlights visual surface irregularities. | **Never generates driving risk scores, driver warnings, or collision collision predictions.** RoadSense is an offline review tool, not an Advanced Driver Assistance System (ADAS). |
| **Repair Priority** | Groups detections temporally. | **Never calculates automated repair priority, municipal budgeting, or engineering urgency.** |
...
```
   [ Raw Frame Detections ]
              │
              ▼
   [ Temporal Clustering ] ──> (Temporal Continuity: Δt < 1.5s, IoU / Centroid Proximity)
              │
              ▼
      [ Proposed Road Event ] ──> (Status: PENDING_REVIEW)
              │
      ┌───────┴───────────────────┐
      │ Reviewer Action           │
      ▼                           ▼                           ▼
